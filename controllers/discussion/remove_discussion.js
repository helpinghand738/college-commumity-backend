...
exports.removeDiscussion = async (req, res) => {
  ...
  try {
    ...
    if (discussion.reference) {
      const removeReplyRes = await this.updateDatabaseToRemoveReply(
        discussion,
        userId
      );
      ...
      const removeDiscussionRes = await this.updateDatabaseToRemoveDiscussion(
        discussion,
        userId
      );
...
exports.updateDatabaseToRemoveVotedUsers = async (discussion) => {
  await User.updateMany(
    ...
    {
      $pull: {
        upDiscussions: discussion._id.toString(),
      },
    }
  );

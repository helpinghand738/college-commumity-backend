...
const {
  updateDatabaseToRemoveDiscussion,
} = require("../discussion/remove_discussion");
  ...
  try {
    ...
    if (post.discussions.length !== 0) {
      const discussions = await Discussion.find({
        _id: { $in: post.discussions },
      });

      for (let discussion of discussions) {
        const removeDiscussionRes = await updateDatabaseToRemoveDiscussion(
          discussion,
          discussion.user
        );
